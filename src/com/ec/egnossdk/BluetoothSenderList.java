...
/**
 * Activity that loads the "Bluetooth Receivers" screen. Loads any paired
 * Bluetooth devices. Also scans for any new Bluetooth devices.Displays spinning
 * progress bar, while the external Bluetooth receiver is connected and
 * identified.Initiates a background process to write and read data from the
 * external Bluetooth receiver, to obtain GPS and EGNOS positions.
 */
public class BluetoothSenderList extends Activity {
	...
	private BluetoothConnectToSend btConnect;
	 ...
	 * onCreate function
	 * 
	 * Called on start of activity, displays scan button and a any paired
	 * bluetooth devices.
	 */
	@Override
	public void onCreate(Bundle savedInstanceState) {
		...
		// register for a broadcast receiver to detect if any new devices are
		// found.
		IntentFilter filter = new IntentFilter(BluetoothDevice.ACTION_FOUND);
		registerReceiver(mReceiver, filter);

		Set<BluetoothDevice> pairedReceivers = bluetoothAdapter
				.getBondedDevices();
		...
		if (pairedReceivers.size() > 0) {
			...
			for (BluetoothDevice device : pairedReceivers) {
				...
				pairedReceiverAdapter.add(deviceName + ":ankur "
						+ deviceAddress);
			}
		}
	...
	/**
	 * scanForReceivers function Starts discovering any new bluetooth devices
	...
	protected void scanForReceivers() {
		...
		if (bluetoothAdapter.isEnabled()) {
			...
			finish();
			Toast.makeText(this, "Turn on Bluetooth", Toast.LENGTH_SHORT)
					.show();
		}
	}
				...
				try {
					...
					if (device.getBondState() != BluetoothDevice.BOND_BONDED) {
						...
						if (checkDuplicate(deviceAddress) == false)
							newReceiverAdapter.add(deviceName + ":"
									+ deviceAddress);
					}
	 ...
	 * @return exist TRUE if device address already exists in new devices
	 ...
	 * 
	 * Listener when an item is clicked from paired device list or new device
	 * list.
	 * 
	...
	private OnItemClickListener receiverClickListener = new OnItemClickListener() {
		@Override
		public void onItemClick(AdapterView<?> av, View view, int id,
				long position) {
			bluetoothAdapter.cancelDiscovery();
			String info = ((TextView) view).getText().toString();

			bluetoothAddress = info.substring(info.length() - 17);
			Log.e("ANKUR", " bluetoothAddress = " + bluetoothAddress);
			
			 // Create the result Intent and include the MAC address
            Intent intent = new Intent();
            intent.putExtra(EXTRA_DEVICE_ADDRESS, bluetoothAddress);
			...
			/*
			 * Ankur here we get a selected device for connection and we will
			 * return to main app for handling
			 */
...
//			progressBarThread.start();
	 ...
	 * 
	 * @param id
	 *            id of the progress bar to be created.
	 ...
	 * 
	 * Handler to take action on different messages. Display or remove progress
	 * bar dialog while receiver is connecting to device. Display or remove
	 * progress bar dialog while receiver is being identified. Display Toast
	 * messages from a non UI thread in this case from uBlox class.
	 */
		...
		public void handleMessage(Message message) {
			...
			case IDENTIFY_RECEIVER:
				...
				if (identifyReceiver) {
					...
					Toast.makeText(getBaseContext(),
							R.string.uBloxReceiverconnected, Toast.LENGTH_SHORT)
							.show();
					...
					Toast.makeText(getBaseContext(),
							R.string.receiverUnIdentified, Toast.LENGTH_SHORT)
							.show();
					removeDialog(0);
					progressBarThread.setState(ProgressBarThread.DONE);
					identifyReceiver = false;
			...
			case uBLOX_MESSAGE:
				Toast.makeText(getApplicationContext(),
						message.getData().getInt(TOAST), Toast.LENGTH_SHORT)
						.show();
				break;
			}
		}
	 ...
	 * 
	 * Thread to connect to a Bluetooth receiver and identify the connected
	 * Bluetooth receiver.
	 */
	static public Handler handle;
	...
	private class ProgressBarThread extends Thread {
		...
		ProgressBarThread() {
			// mHandler = h;
			btConnect = new BluetoothConnectToSend(getBaseContext(),
					bluetoothAddress, handle, tv);
			mState = RUNNING;
		}
		...
		//	progressBarThread.setState(ProgressBarThread.DONE);
			...
			setResult(BluetoothMessageTransferService.CONNECED_TO_SENDER);
	 ...
	 * 
	 * Initiates background process to write and read data from receiver. Parse
	 * the data and send data to SW Receiver to get GPS and EGNOS coordinates.
	 **/
	public class PositionComputation extends AsyncTask<String, Void, Integer> {

		/**
		 * doInBackground function
		 * 
		 * Initiates background process to write and read data to the receiver.
		 * Parse the data and send data to SW Receiver to get GPS and EGNOS
		 * coordinates.
		 * 
		 * @param params
	 ...
	 * onDestroy function
	 * 
	 * Cancels any bluetooth device discovery and unregister any broadcast
	 * receiver when activity is destroyed.
	 */
	@Override
